...
public:
	...
	//DL - Size is not used in the program, so I am commenting it out.
	//DL - this looks to be a residual declaration from the original code.
	//size_t Size();
};
...
void HashTable::Remove(string bidId) {
	...
	Node* cursor = &nodes.at(hash(atoi(bidId.c_str())));


	//DL - Check for found entry to be the first entry in the list
	if (bidId == cursor->bid.bidId) {

		//DL -  if the next node is null, we are at the end of the list
		//DL - check for head to be the last entry
		if (cursor->next == nullptr) {
			//DL - reset the bucket to defaults.
			cursor->bid = Node().bid;
			cursor->key = Node().key;
			cursor->next = nullptr;
			return;
	...
	else
	{
		...
		Node* currentNode = &nodes.at(hash(atoi(bidId.c_str())));
...
Bid HashTable::Search(string bidId) {
	...
	Node* cursor = &nodes.at(hash(atoi(bidId.c_str())));
	//DL -  if entry not found for the key
	if (cursor == NULL) {
		return bid;
	}

	else {
		//DL - cursor is not an empty node, so we need to search for the bidID
		while (cursor->bid.bidId != "") {
			//DL -  if the current node matches the target value, return it
			if (cursor->bid.bidId == bidId) {
				return cursor->bid;
			}
			//DL - node is equal to next node
			cursor = cursor->next;
		}
	}

	//DL - we didn't find a match, return empty bid.
	return bid;
}
